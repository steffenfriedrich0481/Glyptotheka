...
const FileList: React.FC<Props> = ({ categories, images = [] }) => {
  ...
  const renderStlFile = (file: StlFile) => {
    ...
    return (
      <li key={file.id} className="flex justify-between items-center p-2 hover:bg-gray-100 dark:hover:bg-gray-700/20 rounded transition-colors">
        <div className="flex items-center gap-3 flex-1">
          {showPreview ? (
            ...
            <div className="w-16 h-16 bg-gray-200 dark:bg-theme-lighter rounded flex items-center justify-center">
              <span className="text-2xl">🧊</span>
            </div>
          )}
          <span className="truncate text-gray-900 dark:text-theme">{file.filename}</span>
        </div>
  ...
  return (
    <>
      <div className="bg-white dark:bg-theme-lighter shadow rounded-lg p-4">
        {categories.map((category, idx) => (
          <div key={idx} className="mb-6 last:mb-0">
            <h3 className="font-bold text-lg mb-4 text-gray-900 dark:text-theme">
        ...
        {images.length > 0 && (
          <>
            ...
            <ul className="space-y-2">
              {images.map((image) => (
                <li key={image.id} className="flex justify-between items-center p-2 hover:bg-gray-100 dark:hover:bg-gray-700/20 rounded transition-colors">
      ...
      {previewModal && (
        ...
        >
          <div 
            className="relative max-w-5xl max-h-[90vh] bg-white dark:bg-theme-lighter rounded-lg shadow-2xl overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Close button */}
            <button
              onClick={() => setPreviewModal(null)}
              className="absolute top-4 right-4 z-10 bg-theme bg-opacity-50 hover:bg-opacity-75 text-theme rounded-full w-10 h-10 flex items-center justify-center text-2xl transition-all"
            >
              ×
            </button>
            ...
            <div className="flex flex-col items-center p-6">
              ...
              <div className="mt-4 text-center">
                <h3 className="text-xl font-bold text-gray-900 dark:text-theme mb-2">
